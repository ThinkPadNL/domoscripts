...
local TempHumSensor = 'BG | Temp warmwater' 

commandArray = {}
if (devicechanged[TempHumSensor]) then -- Only run rest of script when above sensor is updated in Domoticz

local PipeTemperatureSensor = 'VIRT | Temp warmwater' --NAME of the virtual sensor that only contains temperature
...
local FanSwitch = 'FAN3' --name of switch that controls the fan
local BathroomLightswitch = 'BadkamerLicht' --name of switch of bathroomlight
local FanStandardRuntime = 1800 --time (seconds) that fan should run each shower session, 1800 seconds, half an hour
local FanMaxRuntime = 7200 --time (seconds) that fan should run at max before it is stopped (to prevent running endlessly), 3600 seconds, 1 hour
local TempOnlySensor = 536 --idx of the virtual temperature sensor where you want to store only temperature in
